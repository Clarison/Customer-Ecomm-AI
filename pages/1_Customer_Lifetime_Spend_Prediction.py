...
query = 'select * from CLV where ss_sales_price > 20 and where d_date > (select max(d_date-90) from clv) limit 10000'
...
chart = alt.Chart(df_summary_hist).mark_bar().encode(
    x=alt.X('d_date_purchase_duration:Q', bin=alt.Bin(step=100), title='Duration (days)'),
